...
export class GrabEngine {
  ...
  activate(): void {
    ...
    this.handleMouseMove = (e: MouseEvent) => {
      ...
      this.overlay?.highlight(el, this.getComponentLabel(el))
    }
  ...
  private getVueComponent(el: Element): any {
    ...
    while (node) {
      const instance =
        (node as any).__vueParentComponent || (node as any).__vue_app__?._instance
  ...
  private getComponentLabel(el: Element): string {
